...
function mk_apworld() {
    ...
    echo "From: ${root}"
    echo "To: ${destdir}"
    mkdir --parents "${destdir}/Twilight Princess"
    rsync --progress \
        --recursive \
        --prune-empty-dirs \
        --exclude-from="${CWD}/apworld.ignore" \
        "${root}/" "${destdir}/Twilight Princess"
    pushd "${destdir}"
    zip -9r "Twilight Princess.apworld" "Twilight Princess"
    popd

    rm --force --recursive "${destdir}/Twilight Princess"
}
